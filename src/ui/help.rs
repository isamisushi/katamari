...
pub fn describe(action: Action) -> (&'static str, &'static str) {
    match action {
        ...
        Action::MarkHunkReviewed => ("Diff", "Mark hunk reviewed, advance to the next one"),
        Action::ToggleHunkReviewed => ("Diff", "Toggle this hunk's reviewed mark"),
        Action::MarkFileReviewed => ("Diff", "Mark every hunk in this file reviewed"),
        Action::MarkVisibleReviewed => ("Diff", "Mark every currently visible hunk reviewed"),
        Action::ToggleShowReviewed => ("View", "Show or hide all reviewed hunks expanded"),
        Action::AskAgent => (
            "Agent",
        ...
        Action::CancelAgentTurn => ("Agent", "Cancel the agent's in-flight turn"),
